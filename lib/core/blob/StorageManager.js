...
class StorageManager {
    ...
    listContainer(request, prefix, maxresults) {
        ...
        let result = tables.chain()
            ...
            .limit(maxresults)
            .data();
        return BbPromise.resolve(new AzuriteResponse({ payload: result, cors: request.cors }));
    }
    ...
    getBlob(request) {
        ...
        const response = new AzuriteResponse({ proxy: new BlobProxy(blob, request.containerName), cors: request.cors });
    ...
    listBlobs(request, query) {
        ...
        const response = new AzuriteResponse({ payload: BlobProxy.createFromArray(blobs.limit(query.maxresults).data(), request.containerName), cors: request.cors });
                ...
                .on('finish', () => {
                    ...
                    return BbPromise.all(promises)
                        .then(() => {
                            ...
                            this._clearCopyMetaData(blobProxy);
                            coll.update(blobProxy.release());
                            resolve(new AzuriteResponse({ proxy: blobProxy, cors: request.cors }));
                        });
                });
    ...
    getBlockList(request) {
        ...
        const response = new AzuriteResponse({ proxy: blobProxy, payload: blocks, cors: request.cors });
        return BbPromise.resolve(response);
    }
    ...
    setContainerMetadata(request) {
        ...
        const response = new AzuriteResponse({ proxy: containerProxy, cors: request.cors });
        return BbPromise.resolve(response);
    }
    ...
    putPage(request) {
        ...
        if (endByte >= blobProxy.original.size) {
    ...
    getPageRanges(request) {
        ...
        const response = new AzuriteResponse({ proxy: blobProxy, payload: pageRanges, cors: request.cors });
        return BbPromise.resolve(response);
    }
    ...
    snapshotBlob(request) {
        ...
        return fs.copy(request.uri, snapshotProxy.original.uri)
            .then(() => {
                const response = new AzuriteResponse({ proxy: snapshotProxy, cors: request.cors });
                return response;
            });
    }
    ...
    copyBlob(request) {
        ...
        to.on('finish', () => {
            if (blobProxyDestination.original.copyStatus !== CopyStatus.FAILED) {
                ...
                if (sourceProxy.original.entityType === StorageEntityType.AppendBlob) {
                    blobProxyDestination.original[N.BLOB_COMMITTED_BLOCK_COUNT] = sourceProxy.original[N.BLOB_COMMITTED_BLOCK_COUNT];
                }
        ...
        const response = new AzuriteResponse({ proxy: blobProxyDestination, cors: request.cors });
        return BbPromise.resolve(response);
    }
    ...
    setBlobServiceProperties(request) {
        ...
        if (!settings) {
            ...
            if (updatedSettings.HourMetrics) {
                settings.StorageServiceProperties.HourMetrics = updatedSettings.HourMetrics;
            }
            if (updatedSettings.MinuteMetrics) {
                settings.StorageServiceProperties.MinuteMetrics = updatedSettings.MinuteMetrics;
            }
            ...
            if (updatedSettings.DefaultServiceVersion) {
                settings.StorageServiceProperties.DefaultServiceVersion = updatedSettings.DefaultServiceVersion;
            }
    ...
    _getCopySourceProxy(request) {
        ...
        const resp = request.copySourceName(),
            ...
            date = resp.date;
        if (date !== undefined) {
            const blobProxy = this._getCollectionAndBlob(sourceContainerName, env.snapshotId(sourceContainerName, sourceBlobName, date)).blobProxy;
            return blobProxy;
        }
        const blobProxy = this._getCollectionAndBlob(sourceContainerName, env.blobId(sourceContainerName, sourceBlobName)).blobProxy;
        return blobProxy;
    }
